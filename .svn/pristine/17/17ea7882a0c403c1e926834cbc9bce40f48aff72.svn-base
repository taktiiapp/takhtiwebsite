...
foreach($map as $value){
	//$arra[]=array($value['first_name'],$value['latitude'],$value['longitude'],'https://taktii.live/kZD0rgbzHk/api/profile_image/'.$value['profile_img']);
	$arra[]=array("<a href='abc.com'><b>Name</b>:".$value['first_name']."<br><b>Exp</b>:".$value['total_experience']."</br><b>Address:</b>".$value['address']."</a>",$value['latitude'],$value['longitude'],base_url().'assest/images/marker.png');
}

 $map_json= json_encode($arra);
 ...
 <script type="text/javascript">
  function initialize() {
        ...
        var map = new google.maps.Map(document.getElementById('map'), {
            zoom: 10,
            center: new google.maps.LatLng(28.6128613, 77.1098176),
...
<div class="row">
<br>
<form action="<?=base_url()?>home_controller/search_teacher" method='POST'>
	<div class="col-md-2">
		<select class="form-control" data-style="select-with-transition" onchange="getclass(this.value)" name=search[name]>
	...
	<div class="col-md-2">
		<select class="form-control" data-style="select-with-transition" id="add_class"  onchange="getsubject(this.value);" name=search[class_id] >
		  <option value='' >Select Course First</option>				
		</select>
	</div>

	<div class="col-md-2">
		   <select class="form-control" data-style="select-with-transition" id="add_subject" name=search[subject_id]  >
		  <option value='' > Select Class First </option>	
		   </select>
	</div>

	<div class="col-md-2"> 
	  <select class="form-control" name=search[user_role]>
        <option value='' >Select Tutor Type</option>
        <option value='2'>Home Tutor</option>
        <option value='1'>Institute Tutor</option>
		 <option value=''>Institute</option>
        <option value='3'>Non-Academic Tutor</option>
      </select>
	  </div>
	  	<div class="col-md-2">
	  <input type="text" class="form-control" name=search[address] id="pac-input" placeholder="Enter Location">
	  </div>
	  	<div class="col-md-2">
  ...
  <div class="navbar navbar-default navbar-fixed-top" style="height:70px;">
	...
	<link rel="stylesheet" type="text/css" href="<?= base_url()?>assest/css/bootstrap-datetimepicker.min.css">
	<script src="<?= base_url()?>assest/js/moment.js"></script>
		 ...
		 //loaderStart('Loading...');	 
			data_arr = {'courses_id':courses_id};  
				$.ajax({
					type:'GET',
					data:data_arr,
					url:'<?= base_url()?>home_controller/get_classes_by_course/',
			...
			data_arr = {'class_id':class_id}; 
				$.ajax({
					type:'GET',
					data:data_arr,
					url:'<?= base_url()?>home_controller/get_subjects_by_classes/',
					success:function(response)					 
					{
						//loaderEnd();
